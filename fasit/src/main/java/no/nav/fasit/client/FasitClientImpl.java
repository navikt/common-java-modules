...
public class FasitClientImpl implements FasitClient {
    ...
    public ServiceUserCertificate getCertificate(GetCertificateRequest getCertificateRequest) {
        Document document = fetchXml(format("https://fasit.adeo.no/conf/resources?envClass=%s&type=Certificate&alias=%s&bestmatch=true", getCertificateRequest.environmentClass, getCertificateRequest.alias));
    ...
    public Properties getApplicationEnvironment(GetApplicationEnvironmentRequest getApplicationEnvironmentRequest) {
        ...
        try {
            ...
            try {
                String command = format("sudo cat /app/%s/configuration/environment.properties", getApplicationEnvironmentRequest.applicationName);
