...
import no.nav.common.metrics.MetodeEvent;
...
public class CountAspect {
    ...
    public Object countPaMetode(ProceedingJoinPoint joinPoint, Count count) throws Throwable {
        ...
        String eventNavn = AspectUtil.lagMetodeTimernavn(joinPoint, count.name());
    ...
    public Object countPaKlasse(ProceedingJoinPoint joinPoint, Count count) throws Throwable {
        if (AspectUtil.metodeSkalIgnoreres(AspectUtil.getMetodenavn(joinPoint), count.ignoredMethods())) {
            return joinPoint.proceed();
        }

        AspectMetodekall metodekall = new AspectMetodekall(joinPoint);
        String eventNavn = AspectUtil.lagKlasseTimernavn(joinPoint, count.name());
